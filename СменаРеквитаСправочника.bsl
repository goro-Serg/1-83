...
Процедура КомандаВыбратьСправочник(Команда)
	Значение = ВыбранныйСправочник;
	ПоказатьВводСтроки(Новый ОписаниеОповещения("ПослеВыбораСправочника", ЭтотОбъект),
		Значение, "Имя справочника (как в конфигураторе), напр.: Номенклатура", , Ложь);
КонецПроцедуры
...
Функция ОчиститьЯчейку(Зн)
	...
	Т = "" + Зн; // к строке
	Если СтрДлина(Т) >= 2 И Лев(Т,1) = """" И Прав(Т,1) = """" Тогда
